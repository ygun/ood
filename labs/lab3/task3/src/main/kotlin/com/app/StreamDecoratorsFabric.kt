    ...
    private fun defineDecorator(
        ...
        return when (val currOperation = argsQueue.poll()) {
            Operation.ENCRYPT.toString() -> {
                ...
                EncryptOutputStream(outputSteam, key)
            }
            Operation.DECRYPT.toString() -> {
                val key = argsQueue.poll().toLong()
                DecryptInputStream(inputSteam, key)
            }
            Operation.COMPRESS.toString() -> CompressOutputStream(outputSteam)
            Operation.DECOMPRESS.toString() -> DecompressInputStream(inputSteam)
