...
class HeatIndexDisplay(
    ...
    private var heatIndexIn = 0f
    private var heatIndexOut = 0f
    ...
    override fun updateIn(context: Context) {
        heatIndexIn = calculateHeatIndex(context.temperature, context.humidity)

        display()
    }

    override fun updateOut(context: Context) {
        heatIndexOut = calculateHeatIndex(context.temperature, context.humidity)

        display()
    }

    override fun display() {
        print("Heat index input is $heatIndexIn; ")
        println("Heat index output is $heatIndexOut")
    }
