...
class TestApp {
    ...
    fun `paintPicture draw triangle and rectangle`() {
        ...
        val ins: InputStream = File("./src/test/kotlin/app/paintPicture.txt").inputStream()
    ...
    fun `paintPictureOnModernGraphicsRenderer draw triangle and rectangle`() {
        ...
        val ins: InputStream = File("./src/test/kotlin/app/paintPictureOnModernGraphicsRenderer.txt").inputStream()
        val expectedStr = ins.bufferedReader().use { it.readText() }
